...
export default class LevelC extends Phaser.Scene {
	...
	editorCreate() {
		...
		// clip_a
		const clip_a = this.add.video(640, 360, "3_1");

		// clip_b
		const clip_b = this.add.video(640, 360, "3_2");
		clip_b.visible = false;

		// clip_c
		const clip_c = this.add.video(640, 360, "3_3");
		clip_c.visible = false;

		// clip_d
		const clip_d = this.add.video(640, 360, "3_4");
		...
		// text_3
		const text_3 = this.add.text(207, 10, "", {});
		text_3.setOrigin(0.5, 0.5);
		text_3.text = "D. Because only people born into royalty can hold executive \noffice";
		...
		// text_2
		const text_2 = this.add.text(214, 5, "", {});
		text_2.setOrigin(0.5, 0.5);
		text_2.text = "C. Because the country has a monarch, but his role is symbolic\n and governed by the Constitution";
		...
		// text
		const text = this.add.text(190, 0, "", {});
		text.setOrigin(0.5, 0.5);
		text.text = "B. Because the King has veto power over Jamaican laws";
		...
		// text_1
		const text_1 = this.add.text(214, 0, "", {});
		text_1.setOrigin(0.5, 0.5);
		text_1.text = "A. Because Parliament is required to swear loyalty to the King";
		...
		question.text = "Why is Jamaica described as a “constitutional monarchy”?";
		...
		interdutcion_discription.text = "Dunn's River made sense—'cause birds shouldn’t want to get wet,\nBut with Babylon so determined , this wasn’t a safe bet.";
		...
		interduction_Title.text = "Dunn's River";
	...
	create() {
		...
		this.introductionSound = this.sound.add("level_d_interduction");
		...
		this.dunns_driver = this.sound.add("dunns_driver");
		...
		this.typeText(this.interduction_Title, "Dunn's River", 100);

		// Add typing animation for description after title is complete
		this.time.delayedCall(1200, () => {
			this.typeText(
				this.interdutcion_discription, 
				"Dunn's River made sense—'cause birds shouldn't want to get wet,\nBut with Babylon so determined, this wasn't a safe bet.", 
				30
			);
		});
		...
		this.time.delayedCall(8500, () => {
			...
			this.tweens.add({
				...
				onComplete: () => {
					...
					this.interduction_Title.setText("Dunn's River"); // Ensure text is fully displayed
					this.interdutcion_discription.setText("Dunn's River made sense—'cause birds shouldn’t want to get wet,\nBut with Babylon so determined , this wasn’t a safe bet.");
										...
										this.tweens.add({
											...
											onComplete: () => {
												...
												this.time.delayedCall(8000, () => {
													...
													this.transitionManager.fadeToScene("LevelC", 800);
												});
											}
										});
									});
	...
	stopAllSounds() {
		...
		if (this.dunns_driver) this.dunns_driver.stop();
